package codewars_playing_with_digits;
...
public class DigPow {
    ...
    public static long digPow(int numberToCheck, int firstPotenz) {

        List<Integer> list = new ArrayList<>();
        
        while (numberToCheck > 0) {
            int currentDecimalNumber = numberToCheck % 10;
            list.add(currentDecimalNumber);
            
            numberToCheck /= 10;
        }
        
        int lastPotenz = firstPotenz + (list.size() - 1);
        
        long result = 0;
        for (int i = lastPotenz; i > firstPotenz; i--) {
            result = (long) Math.pow(list.get(i), i) ;
        }        
        
        
        if (numberToCheck % result == 0) {
    ...
    private int calculateSuccessivePowers() {
